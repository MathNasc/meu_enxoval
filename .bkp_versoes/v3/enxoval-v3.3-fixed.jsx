/**
 * ENXOVAL APP v3 — fixed
 * Bug: Storage.get(key, shared, fallback) was called as
 *      Storage.get(key, fallback) → fallback landed in `shared` slot → returned null
 * Fix: signature now Storage.get(key, fallback, shared=false) + null-guards everywhere
 */
...
import {
  ...
  Lightbulb, AlertTriangle, Bell, Award
} from "lucide-react";
  ...
  <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    :root{
      ...
      --f:'Outfit',sans-serif;--fd:'Instrument Serif',serif;
    }
    .dk{
    ...
    .fab{position:fixed;bottom:24px;right:20px;z-index:80;width:54px;height:54px;border-radius:50%;background:var(--p);color:#fff;border:none;cursor:pointer;align-items:center;justify-content:center;box-shadow:0 4px 20px rgba(184,92,50,.45);animation:fabIn .4s cubic-bezier(.34,1.56,.64,1) both;display:none}
  `}</style>
);
...
═══════════════════════════════════════════════════════ */
const AI = {
  call: async (prompt, webSearch = true) => {
    const body = {
  ...
  extractProduct: async (url) => AI.call(`
Acesse esta URL: "${url}"
Retorne APENAS JSON (sem markdown):
{"name":"nome do produto","price":"número decimal ou null","imageUrl":"URL da imagem ou null","brand":"marca ou null","suggestedRoom":"quarto|sala|cozinha|banheiro|outro"}
`, true),

  completeHome: async (rooms, items, aptSize) => AI.call(`
Consultor de enxoval. Apê: ${aptSize}. Cômodos: ${rooms.map(r=>r.name).join(", ")}.
Já tem: ${items.map(i=>i.name).join(", ") || "nenhum"}.
Liste itens ESSENCIAIS faltando. Retorne APENAS JSON array (sem markdown):
...
function generateInsights(items=[], rooms=[], budget=null, deliveryDate="") {
  ...
  if (days !== null && days > 0 && days <= 30)
    insights.push({ type:"alert", Icon:Clock, delay:0, text:`⏰ Faltam apenas ${days} dias! Foque nos itens de alta prioridade primeiro.` });

  if (emptyRooms.length > 0)
    insights.push({ type:"warn", Icon:AlertTriangle, delay:.08, text:`Cômodos sem itens: ${emptyRooms.map(r=>r.name).join(", ")}.` });

  if (highPrio.length > 0) {
    const pct = total > 0 ? Math.round((highPrio.length/total)*100) : 0;
    insights.push({ type:"alert", Icon:Flame, delay:.16, text:`${highPrio.length} item${highPrio.length>1?"s":""} de alta prioridade pendente${highPrio.length>1?"s":""}${pct>0?` (${pct}% do total)`:""}.` });
  }
        ...
        {step === 2 && (
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"flex",gap:8,justifyContent:"space-between",marginTop:4}}>
              <button className="btn btn-g" onClick={()=>{setStep(1);setExtracted(null);setError("");}}>← Voltar</button>
        ...
        {step===2 && (
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"flex",gap:8,justifyContent:"space-between",paddingTop:6,borderTop:"1px solid var(--bdr)"}}>
              <button className="btn btn-g" onClick={()=>setStep(1)}>← Refazer</button>
      ...
      <div style={{display:"flex",gap:12}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:6,alignItems:"center"}}>
            ...
            {item.price && <span style={{fontSize:13,fontWeight:800,color:"var(--p)"}}>{fmt(item.price)}</span>}
          ...
          <div style={{display:"flex",gap:4,alignItems:"center"}}>
            ...
            <button className="btn btn-g bico bdng" onClick={()=>onDelete(item.id)} style={{marginLeft:"auto"}}><Trash2 size={13}/></button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="card" style={{padding:"18px 20px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          ...
          <button className="btn btn-g" onClick={onOpenBudget} style={{fontSize:12}}>
            <Edit3 size={12}/>{budget?.total?"Editar":"+ Orçamento"}
          </button>
        </div>
        {overBudget && (
...
function ItemsView({ items=[], rooms=[], onAdd, onQuickAdd, onToggle, onEdit, onDelete, onDuplicate, onStar }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      <div style={{display:"flex",gap:5,flexWrap:"wrap",alignItems:"center"}}>
        <span style={{fontSize:10,fontWeight:700,color:"var(--tx3)",textTransform:"uppercase",letterSpacing:".07em"}}>Filtros:</span>
        ...
        <button className={`chip ${fStar?"on":""}`} onClick={()=>setFStar(v=>!v)}>⭐ Favoritos</button>
        {[{id:"all",l:"Prio: Todas"},{id:"high",l:"⚡ Alta"},{id:"normal",l:"Normal"}].map(p=>(
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",gap:7,alignItems:"center"}}>
          <span style={{fontSize:12,color:"var(--tx3)"}}>Ordenar:</span>
          ...
          return (
            <div key={r.id} className="card clift au" style={{padding:"18px 20px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
                ...
                {!isDefault(r.id)&&<button className="btn btn-g bico bdng" onClick={()=>onDeleteRoom(r.id)}><Trash2 size={13}/></button>}
              </div>
              {tot>0?(
                <>
...
export default function App() {
  ...
  const dupItem     = useCallback((item)=>setItems(p=>[...p,{...item,id:uid(),status:"want",createdAt:new Date().toISOString()}]),[]);
  ...
  const deleteRoom  = useCallback((id)=>{ if(items.some(i=>i?.roomId===id)){alert("Remova os itens deste cômodo primeiro.");return;} if(confirm("Excluir?")) setRooms(p=>p.filter(r=>r.id!==id)); },[items]);
  ...
  const exportCSV   = useCallback(()=>{
    const h=["Nome","Cômodo","Status","Preço","Prioridade","Loja","Link","Notas"];
    const r=items.map(i=>{ const rm=rooms.find(x=>x.id===i?.roomId); const st=getStore(i?.link); return [i?.name||"",rm?.name||"",i?.status==="bought"?"Comprado":"Quero comprar",i?.price||"",i?.priority||"",st?.n||"",i?.link||"",i?.notes||""]; });
  ...
  const pending = items.filter(i=>i?.status!=="bought").length;
    ...
    <div className={dk?"dk":""} style={{display:"flex",minHeight:"100vh",background:"var(--bg)"}}>
      ...
      <aside className={`sidebar ${sidebar?"open":""}`} style={{width:228,background:"var(--bg2)",borderRight:"1px solid var(--bdr)",padding:"18px 13px",display:"flex",flexDirection:"column",gap:3,zIndex:100}}>
        ...
        {navItems.map(n=>{
          const Icon=n.icon;
          return (
            <button key={n.id} className={`nb ${view===n.id?"on":""}`} onClick={()=>{setView(n.id);setSidebar(false);}}>
        ...
        <div style={{background:"var(--bg3)",borderRadius:11,padding:"13px",marginBottom:10}}>
          ...
          {[
            {l:"Total",    v:items.length,                                c:"var(--tx)"},
            {l:"Comprados",v:items.filter(i=>i?.status==="bought").length, c:"var(--g)"},
            {l:"Pendentes",v:pending,                                      c:"var(--p)"},
            {l:"Favoritos",v:items.filter(i=>i?.starred).length,           c:"var(--go)"},
          ].map((s,i)=>(
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5}}>
          {[
            {Icon:dk?Sun:Moon, fn:()=>setDk(d=>!d), active:false},
            {Icon:Heart,       fn:()=>setShareModal(true), active:couple, ac:"var(--p)"},
            {Icon:Download,    fn:exportCSV, active:false},
          ].map((b,i)=>(
            <button key={i} className="btn btn-g bico" onClick={b.fn}
              style={{background:"var(--bg3)",width:"100%",height:33,borderRadius:9,color:b.active?b.ac:"var(--tx3)"}}>
              <b.Icon size={14} style={b.active?{fill:b.ac}:{}}/>
            </button>
          ))}
        </div>
      </aside>
      ...
      <main style={{flex:1,minWidth:0,display:"flex",flexDirection:"column"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"24px 20px"}}>
          <div style={{maxWidth:900,margin:"0 auto"}}>
            {view==="dashboard"&&<Dashboard items={items} rooms={rooms} deliveryDate={date} onSetDate={setDate} budget={budget} onOpenBudget={()=>setBudgetModal(true)} onQuickAdd={()=>setQuickModal(true)} onAddItem={()=>openAdd()} onCompleteHome={()=>setHomeModal(true)} coupleMode={couple}/>}
            {view==="items"&&<ItemsView items={items} rooms={rooms} onAdd={openAdd} onQuickAdd={()=>setQuickModal(true)} onToggle={toggleItem} onEdit={setItemModal} onDelete={deleteItem} onDuplicate={dupItem} onStar={starItem}/>}
            {view==="rooms"&&<RoomsView rooms={rooms} items={items} onAddRoom={addRoom} onDeleteRoom={deleteRoom}/>}
          </div>
        </div>
      </main>

      {/* FAB */}
      <button className="fab" onClick={()=>setQuickModal(true)}><Plus size={22}/></button>

      {/* Modals */}
      {quickModal  && <QuickAddModal rooms={rooms} items={items} onSave={(d)=>{saveItem(d);setQuickModal(false);}} onClose={()=>setQuickModal(false)}/>}
