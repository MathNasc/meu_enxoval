...
/* ═══════════════════════════════════════════════════════
   STORAGE SERVICE — localStorage para uso local
═══════════════════════════════════════════════════════ */
const Storage = {
...
═══════════════════════════════════════════════════════ */
/**
 * AI SERVICE — usa rotas locais Next.js (sem API key no frontend)
 * Cada função chama /api/* que executa o scraping no servidor.
 */
const AI = {
...
function ItemsView({ items=[], rooms=[], onAdd, onQuickAdd, onToggle, onEdit, onDelete, onDuplicate, onStar, onUpdatePrice }) {
  ...
  const filtered = useMemo(()=>{
    ...
    return arr;
  },[items,search,fRoom,fStatus,fPrio,fStar,fPromo,sort]);
...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      ...
      const [si,sr,sd,sb,sc,sdk] = await Promise.all([
        Storage.get("env:items", []),
        Storage.get("env:rooms", DEFAULT_ROOMS),
        Storage.get("env:date", ""),
        Storage.get("env:budget", null),
        Storage.get("env:couple", false),
        Storage.get("env:dk", false),
      ]);
      ...
      if (sc) {
        const shared = await Storage.get("env:items:shared", []);
  ...
  /* Persist */
  useEffect(()=>{ if(!loading){ Storage.set("env:items", items); if(couple) Storage.set("env:items:shared", items); }},[items,loading,couple]);
   ...
   * ONLY saves the offers list for display — NEVER touches priceHistory
   * (that would corrupt promo detection by recording higher prices as "history").
   * Price update only happens if the best found price is CHEAPER than current.
   */
  const updateItemPrice = useCallback((id, offers) => {
    setItems(prev => prev.map(item => {
      ...
      // Save offers for display only — no history mutation
      let updated = {
        ...item,
        priceOffers: offers,
      ...
      // Only update the item's price (and record history) if found price is cheaper
      if (bestPrice > 0 && (!item.price || bestPrice < curPrice)) {
        updated = recordPrice(updated, bestPrice, "comparison");
        updated = { ...updated, price: String(bestPrice) };
      }
