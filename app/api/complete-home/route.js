 ...
 *
 * FIX: ESSENTIALS[room.id] sempre retornava undefined porque room.id
 * é um UUID do Supabase. Agora normaliza o nome do cômodo para
 * encontrar a chave correta ("quarto", "sala", etc.).
 */
...
/**
 * Normaliza o nome do cômodo para encontrar a chave no ESSENTIALS.
 * room.id no Supabase é UUID — a chave ESSENTIALS é o nome normalizado.
 * Ex: "Quarto Casal" → "quarto", "Sala de Estar" → "sala"
 */
function getEssentialsKey(roomName) {
  ...
  // Match parcial — "quarto casal" → "quarto"
...
export async function POST(req) {
  ...
  for (const room of (rooms || [])) {
    // FIX: usa nome do cômodo normalizado, NÃO o UUID
    const key = getEssentialsKey(room.name);
    if (!key) continue; // cômodo personalizado sem lista padrão

    const essentials = ESSENTIALS[key];

    for (const item of essentials) {
      // Pula se já existe item com nome similar
      ...
      suggestions.push({
        name:           item.name,
        roomId:         room.id, // UUID real do banco
  ...
  // Ordena: alta prioridade primeiro, limita a 20
